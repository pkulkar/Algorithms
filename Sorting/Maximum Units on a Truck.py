...
numberOfBoxesi is the number of boxes of type i.numberOfUnitsPerBoxi is the number
of units in each box of the type i.
    ...
    def maximumUnits(self, boxTypes: List[List[int]], truckSize: int) -> int:
        ...
        for box in sortedBoxes:
            ...
            if boxCount > truckSize:
                required = numberofBoxes - abs(truckSize - boxCount)
                totalUnits += (required * numberofUnits)
                print(totalUnits)
                break
...
"""
Time Complexity: O(n log n) where n is the number items in boxtypes
