...
class Solution:
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        ...
        def get_key(log):
            identifier, log = log.split(" ",maxsplit = 1)
            return (0, log, identifier) if log[0].isalpha() else (1, 0)
        return sorted(logs, key = get_key)

"""
Time Complexity: O(M*N* LogN) N is the number of logs and M is the length of logs in list
